    ...
    public class StoresRepository : BaseRepository, IStoresRepository
    {
        public IEnumerable<Store> OphalenStoresSalesBoeken()
        {
            string sql = @"SELECT St.*, '' AS SplitCol, Sa.*, '' AS SplitCol, B.*
                   FROM Store St
                   LEFT JOIN Sale Sa ON St.Id = Sa.storeId
            ...
            using (IDbConnection db = new SqlConnection(ConnectionString))
            {
                ...
                return GroepeerStores(stores);
            }
        }

        private static IEnumerable<Store> GroepeerStores(IEnumerable<Store> stores)
        {
            var gegroepeerd = stores.GroupBy(store => store.Name);
            List<Store> storesMetSales = new List<Store>();

            foreach (var groep in gegroepeerd)
            {
                var store = groep.First();
                List<Sale> alleSales = new List<Sale>();

                foreach (var s in groep)
                {
                    alleSales.Add(s.Sales.First());
                }

                store.Sales = alleSales;
                storesMetSales.Add(store);
            }

            return storesMetSales;
        }
